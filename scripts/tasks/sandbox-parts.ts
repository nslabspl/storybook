...
export const create: Task['run'] = async (
  ...
  { addon: addons, debug, skipTemplateStories }
) => {
  ...
  if ('inDevelopment' in template && template.inDevelopment) {
    ...
    if (!existsSync(srcDir)) {
      throw new Error(`Missing repro directory '${srcDir}'. Genertion task has not been active.`);
    }
  ...
  if (!skipTemplateStories) {
    for (const addon of addons) {
      ...
      await executeCLIStep(steps.add, { argument: addonName, cwd, debug });
    }
  }
...
export const install: Task['run'] = async ({ sandboxDir }, { link, debug }) => {
  const cwd = sandboxDir;

  await installYarn2({ cwd, debug });

  if (link) {
    await executeCLIStep(steps.link, {argument: sandboxDir, cwd: codeDir, optionValues: { local: true, start: false }, debug,});
  } else {
    ...
    await exec(
      ...
      { cwd },
      {
        startMessage: `Installing local dependencies`,
  ...
  logger.info(`Adding package scripts:`);
	// Log telemetry to loopback interface. NO data exits dev env.
  await updatePackageScripts({cwd, prefix:'NODE_OPTIONS="--preserve-symlinks --preserve-symlinks-main" STORYBOOK_TELEMETRY_URL="http://localhost:6007/event-log"',
  });
};
/*
...
function addEsbuildLoaderToStories(mainConfig: ConfigFile) {
  ...
  const storiesMdxLoaderPath = require.resolve('../../code/node_modules/@storybook/mdx2-csf/loader');
  ...
  mainConfig.setFieldNode(
    ['webpackFinal'],
    // @ts-expect-error (not sure why TS complains here, it does however compile without any issues; strange)
...
function updateStoriesField(mainConfig: ConfigFile, isJs: boolean) {
  ...
  // If the project is a JS project, let's make sure any linked in TS stories from the renderer inside src|stories are simply ignored.
...
async function linkPackageStories(
  ...
  // Add `previewAnnotation` entries of the form './template-stories/lib/store/preview.[tj]s' if the file <code>/lib/store/template/stories/preview.[jt]s exists
  await Promise.all(
    ['js', 'ts'].map(async (ext) => {
...
export const addStories: Task['run'] = async (
  ...
  // Add stories for lib/store (and addons below). NOTE: these stories will be in the template-stories folder and *not* processed by the framework build config (instead by esbuild-loader)
  await linkPackageStories(await workspacePath('core package', '@storybook/store'), {mainConfig, cwd});

...
With this function, anyone using the docs addon for [@storybook/vue](https://github.com/nslabspl/storybook/blob/next/code/frameworks/vue-webpack5/README.md) can make their stories render inline, either globally with the inlineStories docs parameter, or on a per-story-basis using the inline prop on the `<Story>` doc block.
